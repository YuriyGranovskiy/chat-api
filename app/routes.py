...
from app.api_models import AddPersonaBody, ChatsResponseBody, CreateChatBody, CreateMessageBody, GetChatResponseBody, MessagesQuery, MessagesResponseBody, PersonasListResponse, LocationsListResponse, WorldsListResponse, ProfilesListResponse, RegisterBody, TokenResponse, LoginBody, ChatPath, WorldBody, WorldPath, PersonaBody, PersonaPath, LocationBody, LocationPath, CreatedBody, ProfileBody, ProfilePath, ErrorData
from app.models import Message, Chat, MessageType, db, DoesNotExistError, World, Location, Persona
...
def send_message(path: ChatPath, body: CreateMessageBody):
    try:
        message_id = services.create_message(path.chat_id, body.message, 'USER')
        return '', 200
    except DoesNotExistError:
...
def delete_chat(path: ChatPath):
    ...
    try:
        ...
        return ErrorData(error=str(e)).model_dump(), 404
